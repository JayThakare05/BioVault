...
export default function LoginPage() {
  ...
  const { registerAccount, loginAccount, isAuthenticated, performBiometricScan, performBiometricVerify, requestBiometricReset } = useWallet();
  ...
  const handleAuth = async () => {
    ...
    if (mode === 'register' && (!username.trim() || !fullName.trim())) {
    ...
    if (mode === 'reset' && (!username.trim() || !email.trim())) {
      setErrorMsg("Please enter username and recovery email");
      setPhase('error');
      setTimeout(() => setPhase('idle'), 3000);
      return;
    }
    ...
    try {
      if (mode === 'register') {
        ...
        await registerAccount({ username, email, fullName, biometricData: fingerprintId, faceBiometricData: faceId });
      } else if (mode === 'reset') {
        setStatusMsg('Sending reset link...');
        await requestBiometricReset(username, email);
        setPhase('success');
      ...
      <div className="glass-card w-full max-w-sm p-6 sm:p-8 flex flex-col items-center gap-6 animate-fadeInUp" style={{ background: 'rgba(13,17,23,0.9)', border: '1px solid rgba(255,255,255,0.08)', animationDelay: '0.2s', opacity: 0 }}>
        ...
        {mode === 'register' ? (
          <div className="w-full space-y-3 animate-fadeIn">
            ...
            <div>
              <label className="text-xs font-medium" style={{ color: '#8b949e' }}>Email Address</label>
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="satoshi@vault.com" className="input-field w-full px-4 py-2 text-sm mt-1" disabled={phase !== 'idle' && phase !== 'error'} />
            </div>
          </div>
        ) : (
          <div className="w-full space-y-3 animate-fadeIn">
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          {mode === 'reset' && (
            <div className="w-full space-y-3 animate-fadeIn">
              <p className="text-xs text-center mb-4" style={{ color: '#8b949e' }}>
                Enter your username and email to receive a recovery link.
              </p>
              <div>
                <label className="text-xs font-medium" style={{ color: '#8b949e' }}>Recovery Email</label>
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="satoshi@vault.com" className="input-field w-full px-4 py-2 text-sm mt-1" />
              </div>
              <div className="h-6 flex items-center justify-center">
                {phase === 'scanning' && <p className="text-sm animate-pulse" style={{ color: '#00d4ff' }}>{statusMsg}</p>}
                {phase === 'success' && <p className="text-sm animate-fadeIn" style={{ color: '#06ffb4' }}>{statusMsg}</p>}
        ...
        {mode === 'login' && (
          <button 
            onClick={() => { setMode('reset'); setPhase('idle'); }} 
            className="text-xs text-[#7c3aed] hover:text-[#00d4ff] transition-colors"
          >
            Trouble with biometrics? Reset access
          </button>
        )}
        ...
        {mode !== 'reset' ? (
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full">
            ...
            <button
              onClick={() => { setMode('login'); setPhase('idle'); setStatusMsg(''); setErrorMsg(''); }}
